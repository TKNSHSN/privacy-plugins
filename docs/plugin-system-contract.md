> **快照说明**：本文件拷贝自 cc-switch 仓库 `docs/dev/plugin-system-contract.md`
> （MIT License），作为 privacy-replace 所实现插件协议的设计契约快照，供本仓库读者
> 查阅；内容以 cc-switch 上游为准。本插件依赖的核心是 §2.5（用户自定义插件）。
